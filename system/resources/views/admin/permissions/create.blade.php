  ...
  <form action="{{(isset($row))?route($params['route'].".update",$parm):route($params['route'].'.store')}}" 
    ...
    @endif
      <div class="card-body">
        ...
        <div class="form-group">
          <label>Module</label>
          <select name="module_id" class="form-control select2 @error('module_id') is-invalid @enderror" style="width: 100%;">
            <option selected disabled>Select Module</option>
            @foreach($modules as $item)
            <option {{ old('module_id',(isset($row))?$row->module_id:"")==$item->id?"selected":"" }} value="{{$item->id}}">{{$item->name}}</option>
            @endforeach
          </select>
          @error('test_type')
              <span class="invalid-feedback" role="alert">
