...
class DisableLaboursExport implements FromCollection, WithHeadings
{
    private $columns;
    private $status;
    private $districts;
    ...
    public function headings(): array
    {
        ...
        foreach ($this->columns as $key => $column) {
            ...
            } elseif ($column == "disabled_labour.disability") {
                $head = "Disability";
            } elseif ($column == "disabled_labour.disability_percent") {
                $head = "Disability Percent";
            } elseif ($column == "labours.mineral_title") {
                $head = "Mineral Title";
            } elseif ($column == "disabled_labour.fy_year") {
                $head = "Fy Year";
            } else {
                $head = $column;
            }
    ...
    public function collection()
    {
        ...
        if (count($this->columns) > 0) {
            $data = DisableLabour::leftJoin("labours", "labours.l_id", "=", "disabled_labour.l_id")
                ...
                ->where("disabled_labour.fy_year", "$fy->year")->get();
        } else {
            $data = [];
        }
        return $data;
